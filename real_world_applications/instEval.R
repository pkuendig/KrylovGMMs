...
my_data <- InstEval
colnames(my_data) <- c("student", "teacher", "studage", "lectage", "service", "department", "y")

for (j in 1:dim(my_data)[2]) {
  my_data[,j] <- as.numeric(my_data[,j])
}

#Categorical variables
cat_vars <- c("student", "teacher")
group_data <- my_data[,cat_vars]

#Predictor variables
pred_vars <- c("studage", "lectage", "service", "department")
my_data[pred_vars] <- lapply(my_data[pred_vars], factor)
...
#image(ZtZ, xlab="", ylab="", sub ="", main="instEval") 
...
init_cov_pars <- c(0.889, 0.444443, 0.444443)
init_betas <- c(3.206,rep(0,ncol(X)-1))
...
saveRDS(list(all_results=all_results, data_info=data_info), "./instEval.rds")
